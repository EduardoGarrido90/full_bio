...
find /scratch/egarrido/ -user egarrido -mtime +5 2> /dev/null |xargs rm -rf
	...
	~/mongodb/bin/mongod --logpath $mongo_db_folder/log.log --dbpath $mongo_db_folder --port $port --smallfiles --noprealloc --nojournal &
	...
	python ~/local/spearmint/spearmint/main.py .
	python compute_optimum.py .

	~/mongodb/bin/mongod --shutdown --logpath $mongo_db_folder/log --dbpath $mongo_db_folder --port $port --smallfiles --noprealloc --nojournal
